...
const decisions=[
  ...
  sd([17,30],req([p('education.status','eq','enrolled'),p('social.primaryPersonId','eq',null)]),
    ...
    '这段校园关系怎么谈？','后来再看到那张值班表，你会想起关系是从哪句话靠近，或从哪一步退开。',[
      c('把作息和边界说开','你约阿宁在食堂谈清借东西、代报名和各自要留的时间。','那顿饭吃了很久。借东西、代报名和周末怎么过，总算不再靠猜。',{route:'talked',effects:[add('capabilities.boundary',2)],outcomeTags:['social:intent:connect','social:campus:turn'],socialOutcome:{variants:[
  ...
  sd([18,68],req([p('employment.status','eq','employed')]),
    ...
    '今晚去不去？','那次聚餐以后，有人记住你到过，也有人记住你把下班时间留给了自己。',[
      ...
      c('说明有安排，不去','你在群里回了不参加，手上的交接照常做完。','第二天交接照常。饭桌上的话你没听见，能确认的消息还得等项目群或交接记录。',{route:'declinedClearly',effects:[add('capabilities.boundary',2),add('desires.peace.fulfillment',1)],outcomeTags:['social:intent:solitude','social:work:declined']}),
  ...
  sd([20,75],req(),
    ...
    '住处要不要和朋友绑在一起？','钥匙、租约或一句拒绝，后来都比“朋友之间好商量”更管用。',[
      ...
      c('这次不合住','你直说作息、钱或边界不合适，没有拿“再看看”拖着对方。','房子很快租给了别人。你倒松了口气，水电费总算不用拿友情来算。',{route:'declinedHousing',effects:[add('capabilities.boundary',2)],outcomeTags:['social:housing:declined']})
  ...
  sd([18,45],req([p('social.primaryPersonId','eq',null)]),
    ...
    '第一次，要不要见？','那次同城邀约可能只停在一顿饭，也可能留下后来还能叫出名字的人。',[
      c('按约去公共场所','你确认了地点和回程，按约到场。聊得来就多坐一会儿，聊不来也能照原路回去。','后来通讯录里留下多少，取决于那天之后双方还愿不愿继续。',{route:'metOffline',effects:[add('capabilities.riskSense',1)],outcomeTags:['social:intent:connect','social:firstMeeting:offline'],socialOutcome:{variants:[
        variant('offline_once',40,'大家吃完各自回家，群里客气地发了几张照片，此后没有再约。','那次见面只占了一个周末。照片留在群里，此后没有再约。',[add('relationships.network',1)],{outcomeTags:['social:firstMeeting:once']}),
      ...
      c('先在线上多聊一阵','你没有赴约，只在群里继续聊共同兴趣，也没承诺何时线下见。','你们继续在群里聊。哪天会不会见面，谁也没定。',{route:'stayedOnline',effects:[],outcomeTags:['social:intent:connect','social:firstMeeting:online'],socialOutcome:{variants:[
  ...
  sd([45,105],req(),
    ...
    '这次把时间留给谁？','那段空出来的时间，最后去了旧消息、一场见面、独处，或者一个新活动。',[
      ...
      c('明确约一次单独见面','你没有用“大家有空聚”绕过去，而是问对方愿不愿意只和你见一面。','这条路可能进入约会，也可能只确认彼此已经走远。',{route:'askedForDate',requirements:req([p('relationships.activePartnerId','eq',null),p('relationships.partnerStatus','in',['none','divorced','widowed'])]),effects:[],outcomeTags:['social:intent:connect','social:reconnect:date'],socialOutcome:{variants:[
        variant('date_reconnected',35,'对方答应了。你们约在白天见面，也把这次叫作一次约会。','同一个人从旧识走进约会；以后还要不要继续，得留给你们再决定。',[updateSocial({personId:'$actor',tie:'close',turn:'reconnected'}),toDating({personId:'$actor'}),add('pressures.loneliness',-2)],{outcomeTags:['social:bridge:romance','social:reconnect:dateAccepted']}),
