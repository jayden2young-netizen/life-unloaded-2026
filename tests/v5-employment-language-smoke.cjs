...
const OUT=process.env.EMPLOYMENT_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.6.4-employment');
...
(async()=>{
  ...
  try{
    ...
    assert.match(drawer,/销售代表/);
    assert.match(drawer,/固定期限合同/);
    assert.match(drawer,/固定收入加奖金/);
