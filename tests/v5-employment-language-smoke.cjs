...
const OUT=process.env.EMPLOYMENT_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.5.12-employment');
...
(async()=>{
  ...
  try{
    ...
    const forced=await page.evaluate(id=>window.__LIFE_DEBUG__.forceDecision(id),EMPLOYMENT_DECISION.id);
    assert.equal(forced,EMPLOYMENT_DECISION.id);
    await page.waitForTimeout(300);
    assert.equal(await page.locator('.choice-sheet h2').innerText(),EMPLOYMENT_DECISION.prompt);
    assert.deepEqual(await page.locator('[data-choice]').allInnerTexts(),EMPLOYMENT_DECISION.choices.map(choice=>choice.text));
    ...
    assert.equal(run.employment.career,'企业项目岗位');
    assert.equal(run.employment.employerType,'private');
    assert.equal(run.employment.sector,'general');
    assert.equal(run.timeline.at(-1).text,`${EMPLOYMENT_DECISION.choices[1].text}。${EMPLOYMENT_DECISION.choices[1].resultText}`);
    const schedule=run.scheduledConsequences.find(item=>item.sourceDecisionId===EMPLOYMENT_DECISION.id);
    ...
    assert.equal(run.timeline.at(-1).text,EMPLOYMENT_ECHO.choiceOutcomes[EMPLOYMENT_DECISION.choices[1].memoryKey].text);
    ...
    assert.match(drawer,/企业项目岗位/);
    assert.doesNotMatch(drawer,/尚未进入社会/);
    await page.waitForTimeout(900);
    await page.screenshot({path:path.join(OUT,'employment-drawer-360x773.png'),fullPage:true});
    ...
    console.log(JSON.stringify({ok:true,decision:EMPLOYMENT_DECISION.id,choice:EMPLOYMENT_DECISION.choices[1].id,consequence:schedule.eventId,screenshots:fs.readdirSync(OUT).sort(),errors},null,2));
  }finally{
    await browser.close();
  }
