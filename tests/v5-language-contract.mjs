...
for(const [trackId,copy] of Object.entries(TRACK_COPY)){
  ...
  const expectedDecisions={education:11,habits:30,business:8,remote:7,partnership:10,children:10,later:9}[trackId]||8;
  check(copy.decisions.length===expectedDecisions,`${trackId}: authored decision count is not ${expectedDecisions}`);
  check(copy.decisions.every(item=>item.choices.length>=3&&item.choices.length<=4),`${trackId}: decision choice count is outside 3-4`);
}
