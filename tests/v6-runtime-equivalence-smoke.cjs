...
(async () => {
  assert.deepEqual(
    ...
    [11, 22],
    'equivalence fixture only applies to the v0.6.4 Schema 11 runtime',
  );
  ...
  if (RECORD) {
    fs.writeFileSync(FIXTURE, `${JSON.stringify(output, null, 2)}\n`);
    console.log(JSON.stringify({ recorded: FIXTURE, checkpoints: trace.length }));
    return;
  }
