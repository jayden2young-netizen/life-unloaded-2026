...
const OUT=process.env.FULL_TRACK_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.6.2-full-track');
...
(async()=>{
  assert.equal(data.version,'0.6.2');
  ...
  try{
    ...
    assert.equal(migrated.gameVersion,'0.6.2');
