...
const OUT=process.env.FULL_TRACK_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.6.1-full-track');
...
(async()=>{
  assert.equal(data.version,'0.6.1');
  assert.equal(data.schemaVersion,11);
  assert.equal(data.contentRevision,20);
  ...
  try{
    ...
    assert.equal(migrated.gameVersion,'0.6.1');
