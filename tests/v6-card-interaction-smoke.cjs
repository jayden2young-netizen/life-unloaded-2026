...
const OUT=process.env.CARD_INTERACTION_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.6.5-cards');
...
(async()=>{
  ...
  try{
    ...
    assert.deepEqual([migrated.schemaVersion,migrated.gameVersion,migrated.run],[11,'0.6.5',null]);
