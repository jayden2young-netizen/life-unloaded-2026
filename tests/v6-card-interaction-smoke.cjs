...
const OUT=process.env.CARD_INTERACTION_SMOKE_OUT||path.join(os.tmpdir(),'life-unloaded-v0.6.0-cards');
...
(async()=>{
  assert.deepEqual([data.version,data.schemaVersion,data.contentRevision],['0.6.0',11,20]);
  ...
  assert.deepEqual(data.cardInteractionCoverage,{decisionPanels:162,activePanels:162,interactions:163,witnesses:1});
  ...
  try{
    ...
    assert.deepEqual([migrated.schemaVersion,migrated.gameVersion,migrated.run],[11,'0.6.0',null]);
