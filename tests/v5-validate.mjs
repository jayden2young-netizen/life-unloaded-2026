...
const choiceFields=['id','text','resultText','hints','requirements','effects','commitments','consequences','outcomeTags','memoryKey','route'];
...
const allowedPrefixes=['age','world.','originHousehold.','education','education.','roles','employment.','activity.','finance.','housing.','relationships.','health','health.','habits.','capabilities.','mobility.','business.','later.','pressures.','desires.','legacy.','agency'];
...
check(data.version==='0.5.9'&&data.gameVersion==='0.5.9','version is not 0.5.9');
check(data.schemaVersion===8,'schemaVersion is not 8');
check(data.contentRevision===16,'contentRevision is not 16');
check((kinds.beat||[]).length===400,'annual beats must remain 400');
check((kinds.decision||[]).length===124,'v0.5.9 must contain 124 decisions');
...
check(data.events.length===668,'v0.5.9 causal coverage must total 668 nodes');
...
const episodeExpectations={
  ...
  ,secondary_diversion:{roles:['start'],deadline:1,routes:['academic','vocational','employment','invalidated']}
  ,university_interruption:{roles:['start','resolve'],deadline:2,routes:['resumed','alternate_completed','working_exit','invalidated']}
...
for(const decision of kinds.decision||[]){
  ...
  if(episodeId==='university_interruption'&&decision.episode.role==='start')check(hasPredicate(decision,'education.path','eq','college')&&hasPredicate(decision,'education.status','eq','enrolled'),`${decision.id}: university interruption lacks enrollment`);
...
for(const card of data.cards){
  keys(card,['id','kind','drawAge','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===16,`${card.id}: stale revision`);
