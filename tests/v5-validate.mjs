...
check(data.version==='0.5.11'&&data.gameVersion==='0.5.11','version is not 0.5.11');
check(data.schemaVersion===9,'schemaVersion is not 9');
check(data.contentRevision===18,'contentRevision is not 18');
check((kinds.beat||[]).length===408,'v0.5.11 must contain 408 annual beats');
check((kinds.decision||[]).length===127,'v0.5.11 must contain 127 decisions');
...
check(data.events.length===682,'v0.5.11 causal coverage must total 682 nodes');
...
for(const event of kinds.beat||[]){
  ...
  if(/产检/.test(event.text)){check(hasPredicate(event,'relationships.parenthoodIntent','eq','planned'),`${event.id}: prenatal scene lacks a planned-parenthood gate`);check(hasPredicate(event,'relationships.activePartnerId','truthy'),`${event.id}: prenatal scene lacks an active partner gate`);check(!event.actors.some(actor=>actor.relationAny?.includes('child')),`${event.id}: prenatal scene is bound to an existing child`)}
...
for(const decision of kinds.decision||[]){
  ...
  if(decision.episode?.id==='secondary_diversion')check(decision.ageMin>=13&&decision.ageMax<=18,`${decision.id}: secondary-school choice is outside adolescence`);
  if(decision.episode?.id==='retirement_transition'||(!decision.episode&&decision.track==='later'&&decision.ageMin>=55))check(decision.ageMin>=50,`${decision.id}: retirement choice is too young`);
  if(decision.episode?.id==='adolescence_boundary')check(decision.actors.some(actor=>actor.ageMin>=10&&actor.ageMax<=20),`${decision.id}: adolescent child has no matching age`);
  if(decision.episode?.id==='adult_child_boundary')check(decision.actors.some(actor=>actor.ageMin>=18),`${decision.id}: adult-child choice lacks adult actor`);
}
...
for(const card of data.cards){
  keys(card,['id','kind','drawAge','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===18,`${card.id}: stale revision`);
