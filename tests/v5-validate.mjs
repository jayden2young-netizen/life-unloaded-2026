...
const commandFields=['type','target','value','kind','rate','guaranteed','relation','status','replace'];
const commandTypes=['add','set','tag','addLiability','createPerson','transition','claimDesire'];
...
const allowedPrefixes=['age','world.','originHousehold.','education','education.','roles','employment.','activity.','finance.','housing.','relationships.','health.','habits.','capabilities.','mobility.','business.','pressures.','desires.','legacy.','agency'];
...
check(data.version==='5.0.0'&&data.gameVersion==='5.0.0','version is not 5.0.0');
...
for(const card of data.cards){keys(card,['id','kind','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===6,`${card.id}: stale revision`)}
