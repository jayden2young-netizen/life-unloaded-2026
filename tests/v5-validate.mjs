...
check(data.version==='0.5.7'&&data.gameVersion==='0.5.7','version is not 0.5.7');
check(data.schemaVersion===8,'schemaVersion is not 8');
check(data.contentRevision===14,'contentRevision is not 14');
check((kinds.beat||[]).length===400,'annual beats must remain 400');
check((kinds.decision||[]).length===121,'v0.5.7 must contain 121 decisions');
check((kinds.consequence||[]).length===121,'every major decision must own a consequence');
check((kinds.blackSwan||[]).length===20,'black swans must remain 20');
check(data.events.length===662,'v0.5.7 causal coverage must total 662 nodes');
...
for(const decision of kinds.decision||[]){
  check(decision.choices.length===(decision.episode?.role==='resolve'?4:3),`${decision.id}: unexpected choice count`);const signature=decision.choices.map(choice=>choice.text).join('|');check(!texts.has(signature),`${decision.id}: repeated choice set`);texts.add(signature);
...
check(JSON.stringify(shopEpisodes.map(event=>event.id))==='["decision_033","decision_034","decision_035"]','shop episode IDs shifted');
...
const episodeExpectations={
  ...
  guarantee_recourse:{ids:['decision_065','decision_068','decision_069'],roles:['start','continue','resolve'],deadline:3,routes:['recovered','restructured','relationship_break','default_failure']},
  acute_illness:{ids:['decision_072','decision_073','decision_074','decision_075'],roles:['start','continue','continue','resolve'],deadline:4,routes:['cured','managed','limited','treatment_exit']}
};
for(const[type,start]of Object.entries({gambling:80,alcohol:86,gaming:92,shopping:98,medication:104}))for(const[kind,offset,routes]of[['formation',0,['stopped','exposed','dependent','uncontrolled']],['treatment',2,['recovery','continuing','support_exit','uncontrolled']],['relapse',4,['recovery_reset','treatment_return','lapse_only','relapse']]])episodeExpectations[`habit_${type}_${kind}`]={ids:[`decision_${String(start+offset).padStart(3,'0')}`,`decision_${String(start+offset+1).padStart(3,'0')}`],roles:['start','resolve'],deadline:2,routes};
...
for(const card of data.cards){
  keys(card,['id','kind','drawAge','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===14,`${card.id}: stale revision`);
