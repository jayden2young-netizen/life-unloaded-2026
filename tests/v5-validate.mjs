...
check(data.version==='0.5.4'&&data.gameVersion==='0.5.4','version is not 0.5.4');
check(data.schemaVersion===7,'schemaVersion is not 7');
check(data.contentRevision===11,'contentRevision is not 11');
check((kinds.beat||[]).length===400,'annual beats must remain 400');
check((kinds.decision||[]).length===107,'v0.5.4 must contain 107 decisions');
check((kinds.consequence||[]).length===107,'every major decision must own a consequence');
check((kinds.blackSwan||[]).length===20,'black swans must remain 20');
check(data.events.length===634,'v0.5.4 causal coverage must total 634 nodes');
...
for(const track of tracks){
  const beats=(kinds.beat||[]).filter(event=>event.track===track),decisions=(kinds.decision||[]).filter(event=>event.track===track&&event.arc),expectedDecisions=track==='habits'?15:8;check(beats.length===32,`${track}: needs 32 authored beats`);check(decisions.length===expectedDecisions,`${track}: needs ${expectedDecisions} arc decisions`);check(JSON.stringify(data.trackCoverage[track]?.roles)==='["entry","development","daily","conflict","crisis","recovery","exit","legacy"]',`${track}: incomplete coverage roles`);
...
for(const card of data.cards){
  keys(card,['id','kind','drawAge','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===11,`${card.id}: stale revision`);
