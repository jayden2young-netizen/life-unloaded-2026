...
check(data.version==='5.0.3'&&data.gameVersion==='5.0.3','version is not 5.0.3');
check(data.schemaVersion===7,'schemaVersion is not 7');
check(data.contentRevision===7,'contentRevision is not 7');
...
for(const card of data.cards){keys(card,['id','kind','displayName','text','mechanic','effects','contentRevision'],card.id);inspectCommands(card.effects,card.id);check(card.contentRevision===7,`${card.id}: stale revision`)}
