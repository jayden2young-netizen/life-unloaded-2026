...
const out=path.join(root,'test-results','v0.4.1-real-life-storylines');
...
(async()=>{
  ...
  original.schemaVersion=5;original.gameVersion='0.4.0.1';original.run.schemaVersion=5;original.run.gameVersion='0.4.0.1';original.run.contentRevision=4;
  ...
  await page.reload({waitUntil:'networkidle'});
  await page.waitForFunction(()=>window.__LIFE_BOOTED__===true);
  let migrated=await page.evaluate(()=>({save:JSON.parse(localStorage.getItem('life-unloaded-2026-v1')),backup:JSON.parse(localStorage.getItem('life-unloaded-2026-v0.4.0.1-backup'))}));
  assert(migrated.backup.schemaVersion===5,'schema 5 backup missing');assert(migrated.save.schemaVersion===6&&migrated.save.gameVersion==='0.4.1','schema 5 save was not upgraded');assert(migrated.save.run.seed===original.run.seed&&migrated.save.run.phase===original.run.phase,'active life was not preserved');assert(migrated.save.meta.histories[0].title==='迁移保留标记','meta history was not preserved');
  ...
  console.log(JSON.stringify({version:'0.4.1',migration:{backup:5,current:6,runPreserved:true},storylines:chosen,settlement:{splitBody:splitSettled.pressures.body,onsiteBody:onsiteSettled.pressures.body,businessCash:businessAfter.res.cash},ending:{age:finalSave.run.age,profile:finalSave.run.ending.profileId,decisions:finalSave.run.decisionCount,review:finalSave.run.ending.review},codexUnlocked:finalSave.meta.codex.length,errors,screenshots:out},null,2));
  await browser.close();
