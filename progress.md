...
- 当前发布基线与本地版本：v0.6.8／Schema 12／Content Revision 26。
- v0.6.8 住房与居住选择已发布；下一步是 v0.6.9 社会交往的开发前规划。
- 下一规划版本是 v0.6.9 社会交往；v0.6.10 必须等待它完成。
...
- 2026-08-08: 修复 v0.6.8 教育路线被首职申请抢占的问题。初中完成后先进入中考分流，高中／职校完成后先进入本科申请；只有玩家明确选择就业出口时首职申请才可达。教育入口会定向优先于已到期活动 episode，并把原事件顺延而不丢失；同版本旧活动人生会修复错误的 `nextStage`，但保留已经作出的就业选择。热修提交 `b78ea26` 已推送到 `main`，GitHub Pages 运行 `31263523633` 成功；线上 `game.js` 与本地逐字节一致，线上 `data.json` SHA-256 为 `FA589969878103C167D6D69987AE3F77E0386E5A839B912D7E9566BF34B10936`。生成器连续两次无差异，事件总数仍为 870；36 文件 syntax、correctness、family-education、university-career 和 employment 均通过，三组真实浏览器检查控制台错误为 0。
- 2026-08-08: v0.6.8“住房与居住选择”以功能提交 `4e5d6d6` 发布到 `main`，`legacy/0.6.7` 固定在发布前的 `main@665431e`，两者均已推送。GitHub Pages 运行 `31260886809` 成功；线上页面、运行时、内容合同和生成数据均为 v0.6.8／Schema 12／Content Revision 26，包含 870 个事件、72 张卡牌、32 项图鉴，线上 `data.json` SHA-256 为 `639C18ED9D2081E1866A7E19AABE831B6C26ACA432A80E1F52C1D8C6279F29D8`，与本地一致。发布同时加入 macOS 一键启动 Python Server 与默认浏览器的 `start-local-play.command`。
- 2026-08-08: 实施 v0.6.8“住房与居住选择”。Schema 升至 12 并清除旧活动人生，保留档案、图鉴、设置与跨局记录；新增原子 `transitionHousing`、住房历史和三次普通选择／一次债务例外预算。地区锚点、租住和按揭负担会读取现金、收入稳定、债务与真实同住伴侣，房值签约后固定，不做年度房价模拟。新增 32 beats、6 decisions，删除旧的全国通用租／买／回家题，并把教育宿舍、公共岗位宿舍、远程基地、伴侣住处、债务处置和晚年照护接入统一住房写入。生成数据为 456 beats、197 decisions、197 consequences、20 black swans，共 870 个事件；生成器双跑 SHA-256 均为 `639C18ED9D2081E1866A7E19AABE831B6C26ACA432A80E1F52C1D8C6279F29D8`。`core,debt,episodes` 通过 syntax、correctness、browser、full-track 与 debt，覆盖 Schema 12 迁移、住房代表纵切片、三次普通选择与一次债务例外、三种窄屏、按揭结清和房屋处置；未执行任何 Git 写操作或发布。
...
- 2026-07-31: 调整 v0.6.x 后续顺序：v0.6.7 改为晚年生活，新增 v0.6.8 住房与 v0.6.9 社会交往两个占位版本，原 v0.6.7 跨系统验收与收口顺延为 v0.6.10。三个新增内容版本只确定主题与边界，详细机制、事件、数值和验收待各版本开发前另行规划。
- 2026-07-30: v0.6.4 首份工作分层与职业系统融合重做在 `codex/v0.6.4-first-job-bridge` 完成本地实现，提交为 `92e4123`，尚未推送、合并或部署。新增统一 `employmentCatalog`、原子职业状态命令、T0—T4 内部分层、地区／薪资档位与五类收入规则；首份工作第三阶段会实际展示 E01—E14 招聘／HR 场景并锁定 offer，第四阶段只接受、拒绝或继续寻找，T4 不可作为首职。医生、律师、大学讲师通过简化资格凭证进入 T3，32 岁后使用三阶段再入场链。E01—E14 招聘场景、16 个完整职业选择和 E63—E72 桥接共 40 个研究种子均已编码。版本为 v0.6.4、Schema 11、content revision 22；数据为 408 beats、183 decisions、183 consequences、20 black swans，双跑 SHA-256 均为 `EC7AA0B8D7BBB16FFC2CD1B16E6A189871EE63395D49DE94870C5D97C6B4B663`。career、cards、core、episodes 与 runtime-refactor profile 通过；E01—E14 均有固定运行时 witness，专业资格正反路径、停薪留职恢复、限期交接和关副业保留主职均有断言；覆盖三种窄屏且控制台错误为 0。
...
- 2026-07-28: v0.6.2 功能提交为 `cec0246`；人工复测通过后开发分支已推送，`legacy/0.6.1` 固定在原 `main@32fb143`，`main` 通过 fast-forward 发布且没有合并提交或 PR。GitHub Pages 运行 `30368740260` 成功，线上 `index.html`、`game.js`、`data.json` 与共享合同均返回200并显示 v0.6.2／Schema 11／Content Revision 20；线上数据 SHA-256 与提交中的 `1A2182D4B3CFC257EB6AEC3270468F9F00874DEF3BF363B78812D4B82A20F09F` 一致。
...
- 2026-07-28: v0.6.1 功能提交为 `2a3910c`；开发分支已推送，`legacy/0.6.0` 固定在原 `main@4b23df7`，`main` 通过 fast-forward 发布且没有合并提交或 PR。GitHub Pages 构建成功，线上 `index.html`、`game.js`、`data.json` 均返回200并一致显示 v0.6.1／Schema 11／Content Revision 20。
- 2026-07-28: 完成 v0.6.x 规划审计与文档重构，没有修改游戏代码、数据、玩家文案或测试。v0.6.0 保持已完成基线；v0.6.1—v0.6.2 分别规划为可审查源码基线和正确性／内容护栏；原教育、首职、家庭、债务、整体验收五个版本按原相对顺序迁移至 v0.6.3—v0.6.7，每个版本继续保留独立 roadmap。
- 2026-07-28: 作者键被严格限制为生成器内部稳定定位和作者配置，不输出到 `data.json`，不进入运行时、存档、历史、玩家数据或跨版本接口，也不形成第二套身份系统。完整稳定语义 ID、因果规则、provenance bus、Headless 模拟和模块化构建均不作为 v0.6.x 前置项。
- 2026-07-28: v0.6.x 模型委托更新为 v0.6.1／v0.6.2／v0.6.7 使用 GPT-5.6 Sol High，v0.6.3—v0.6.6 使用 GPT-5.6 Terra High。四个内容版本必须当版交付可发布文案，v0.6.7 再进行一次 Sol High 级别的整体文案核对；Luna High 不作为主执行者。
- 2026-07-28: 根目录 `CopyWriting_Guideline.md` 已依据第三方审计刷新并无损精简，保留破折号、短句、直接引语、重复句式及幽默／暗黑比例作为诊断信号而非机械失败阈值；没有改动任何现有玩家文案。
- 2026-07-28: v0.6.0 卡牌参与选择完成：Schema 升至11、content revision 升至20。72张卡、四次三选一和抽取时的一次性效果保持不变；持有卡会按最早获得的同机制卡参与后续选择，不消耗、不叠加，也不突破年龄、学历或现实资格门槛。
- 2026-07-28: 所有162个决策面板现在显式生成 `mechanicTags` 与 `cardInteraction`，生成器硬性核验12种机制的阶段／选项覆盖、35／55岁后续用途、75%面板覆盖和一条卡牌专属入口见证。新增浏览器 smoke 覆盖五种交互模式、Schema 10迁移、无卡兼容、同机制主卡、刷新不重复结算及减少动态效果；现有五项核心 smoke 全部通过，控制台错误为0。
- 2026-07-26: 当前任务从干净的 `main@2076ea3` 创建 `codex/v0.5.12-university-to-first-job`，实现本科在读、休学／转学、研究生申请与完整就读、海外华人生活以及本科直就业／研究生就业两类首份工作衔接；不重做本科申请，不改 UI，不自动合并或部署。
- 2026-07-26: 已完成国内学籍与2026考研／就业流程、美国与欧洲留学机制及华人在外融入和歧视体验的案头研究，写入 `docs/research/v0.5.12-本科研究生与首份工作.md`。同胞关系与本地关系按互补证据建模，歧视只记录环境遭遇和压力，不作为录取或就业失败乘数。
- 2026-07-26: v0.5.12 实现完成：Schema 升至10、content revision 升至19，新增12条本科—研究生—首份工作短簇及教育、海外生活、研究生申请和就业衔接状态。休学会暂停原本科主线，复学／转专业／转学结果受原申请约束；本科退出保留为“中断”而非伪造毕业。海外首职解析记录并读取独立工作资格、申请入口、岗位证据、语言、关系和地区，不读取歧视负荷。
- 2026-07-26: 生成数据为408个年度事件、162个选择、162个选择特定长期后果和20个黑天鹅，共752个节点；生成器连续两次 SHA-256 均为 `461F81D2BE5B9F6E86D6485466D4053286CF1896ECF81BB1C24B845CA4330290`。JS语法、教育—职业定向 smoke 和现有四项核心 smoke 通过；覆盖Schema 9迁移、休学复学、转学、申请失败、资助缺口、工作资格受限、持续求职、海外关系网及360×773、360×640、320×568，控制台错误为0。四张代表截图已人工目检，没有运行批量人生模拟。
