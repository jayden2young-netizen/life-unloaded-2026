...
const VERSION='0.5.5',SCHEMA_VERSION=8,CONTENT_REVISION=12;
...
const decisionEffects=(id,index,option,authoredDecision)=>{
  const episodeRoutes={1:['verified','independent_test','guaranteed'],2:['lean','independent_plan','double_down'],3:['survived','independent','stop_loss','debt_failure']},route=authoredDecision?.episode?episodeRoutes[authoredDecision.episode.phase][option]:['deliberate','negotiated','risk'][option],effects=[c('tag','history',`${id}:${route}`),c('add','agency',[4,3,2,1][option])],add=(path,value)=>effects.push(c('add',path,value)),set=(path,value)=>value!==undefined&&effects.push(c('set',path,value));
  ...
  const habitStage=authoredDecision?.phase===3?{0:'recovery',1:'treatment',2:'relapse'}[option]:null,outcomeTags=id==='habits'?['habits',`habits:${authoredDecision.type}`,`habits:${route}`,habitStage?`habits:${habitStage}`:'habits:turn',...(habitStage==='recovery'?['recovery']:[])]:authoredDecision?.episode?['business',`business:${route}`,`episode:${authoredDecision.episode.id}`]:[id,`${id}:${route}`,index===7?`${id}:legacy`:`${id}:turn`];
