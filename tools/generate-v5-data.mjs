...
const VERSION='5.0.0',SCHEMA_VERSION=7,CONTENT_REVISION=6;
...
TRACKS.health=track('健康与照护','personal',[0,105],[],['体检报告折起的一角','药盒里剩下的最后一格','候诊区滚动的号码','请假单上的诊断证明','康复室的弹力带','凌晨亮着的心率记录','家属签字栏的空白','床头柜上的护理计划'],['让日程突然安静。','比疼痛更早提醒你。','旁边写着下一次复查。','后来改变了生活半径。'],['一次检查发现需要继续观察的指标。','治疗会影响工作，但拖延可能加重问题。','长期疲劳开始影响判断和关系。','医生建议你改变饮食、睡眠和活动。','一次事故留下需要康复的功能受限。','精神状态让你很难维持原来的生活。','家人希望替你做决定，你仍想保留自主。','晚年照护需要在家庭、社区和机构间选择。'],[['完成复查建立记录','先观察身体变化','忽略结果继续生活'],['请假接受治疗','调整工作边治疗','拖延到无法回避'],['停止部分责任恢复','寻求专业帮助','靠意志继续硬撑'],['逐项改变生活习惯','只服药不改生活','拒绝长期管理'],['坚持康复训练','接受辅助工具调整生活','放弃康复减少外出'],['接受治疗和支持','只向可信的人求助','继续隐瞒状态'],['写下医疗意愿和授权','完全交给家人决定','拒绝讨论后续'],['购买正式照护服务','主要依靠家庭照护','独自生活承担风险']]);
...
const TRACK_NODE_AGES={
  ...
  health:[[0,70],[12,75],[16,80],[18,85],[18,85],[18,90],[35,100],[55,105]],
...
const decisionEffects=(id,index,option)=>{
  ...
  if(id==='finance'){add('finance.cash',[-6000,-15000,8000][option]);add('pressures.money',[-3,1,6][option]);if((index===0&&option===1)||(index===1&&option>0)||(index===2&&option>0))effects.push(c('addLiability','finance.liabilities',index===0?260000:option===2?120000:40000,{kind:index===0?'mortgage':'consumer',rate:option===2?.12:.06,guaranteed:index===1}));if(index===0)set('housing.status',['renting','mortgaged','family'][option])}
  if(id==='health'){add('health.physical',[6,3,-6][option]);add('health.mental',[4,1,-4][option]);add('pressures.body',[-6,-2,6][option]);add('finance.cash',[-8000,-3000,0][option]);if(index===4&&option===2)set('health.disability','persistent')}
...
const endingProfiles=[
  ...
  ['familyCycle','你成功保住所有退路，因此从未真正离开。','罕见',['children:negotiated','finance:risk']],
  ['cycleBreaker','你没有摆脱家庭，只让下一代少交了一张报表。','极罕',['children:deliberate','finance:deliberate']],
