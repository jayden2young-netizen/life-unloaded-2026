(async()=>{
  ...
  function createRun(seed=makeSeed()){
    ...
    run.employment={status:'student',career:'在读',sector:'education',tenure:0,unemployedYears:0};
    ...
    run.memories=[];run.pressures={money:0,family:0,career:0,body:0,loneliness:0};run.scheduledEchoes=[];run.flags={};run.outcomeTags={};run.crisisYears={employment:0,money:0,health:0};
    ...
    run.startNet=run.res.assets+run.res.cash-run.res.debt;run.ending=null;syncDerivedFacts(run);return run;
  }
  ...
  function loadState(){
    ...
    try{const parsed=JSON.parse(raw);base.meta=normalizeMeta(parsed.meta||{});state=base;const oldSchema=parsed.schemaVersion||parsed.run?.schemaVersion||3;if(oldSchema<SCHEMA_VERSION){if(!localStorage.getItem(V32_BACKUP_KEY))localStorage.setItem(V32_BACKUP_KEY,raw);base.meta.migrationNotice=true;base.run=null}else base.run=migrateRun(parsed.run);return base}
  ...
  function crisisCandidates(type){
    if(!type)return[];return(INDEX.byStage[stageForAge(state.run.age)]?.decision||[]).filter(event=>eligible(event)).filter(event=>event.choices.some(choice=>{
  ...
  function dueFamilyEcho(){const run=state.run,text=run.birth.family.archetype.lateEcho;if(run.familyEchoDone||run.age<68||!text)return null;return{id:`family_echo_${run.birth.family.archetype.id}`,kind:'familyEcho',ageMin:68,ageMax:105,icon:'↩️',text,runtimeText:text,theme:'family',intensity:'medium',effects:{relationships:{parentsBond:3},desires:{familyBelonging:3},outcomeTagsAdd:['familyEcho']},contentRevision:4}}
  ...
  function beginYear(){
    ...
    if(!primary){primary={id:`quiet_${run.age}`,kind:'beat',icon:'·',text:'这一年没有大事，日子照常往前。',theme:'ordinary',effects:{},contentRevision:4}}
  ...
  function fragmentEligible(fragment,run,profile){const req=fragment.requirements||{};if(req.locationAny&&!req.locationAny.includes(run.birth.location.id))return false;if(req.conflictAny&&!req.conflictAny.some(id=>run.mainConflicts.includes(id)))return false;if(req.outcomeTagsAny&&!req.outcomeTagsAny.some(tag=>tagCount(run,tag)||finalSignal(run,tag)))return false;if(req.endingProfileAny&&!req.endingProfileAny.includes(profile.id))return false;return true}
  function endingFragment(group,run,profile){const eligible=(DATA.endingFragments[group]||[]).filter(x=>fragmentEligible(x,run,profile)),pool=eligible.length?eligible:DATA.endingFragments[group]||[];return pool[stable(run.seed,`fragment-${group}`,Math.max(1,pool.length))]?.text||''}
  function finishLife(){
  ...
  function renderGameToText(){const run=state.run;return JSON.stringify({view:state.view,version:VERSION,codexUnlocked:state.meta.codex.length,run:run?{phase:run.phase,age:run.age,canAdvance:state.view==='game'&&run.phase==='playing',overlay:run.phase==='decision'?'decision':run.phase==='card'?'card':null,resources:run.res,employment:run.employment,relationships:{partner:run.relationships.partner,children:childAges(run),support:Math.round(supportScore(run))},topDesires:topDesires(run).map(x=>({key:x.key,fulfillment:Math.round(x.fulfillment)})),pressures:run.pressures,scheduledEchoes:run.scheduledEchoes.map(x=>({eventId:x.eventId,dueAge:x.dueAge})),secretRevealed:run.secretRevealed,visibleTimeline:run.timeline.slice(-6).map(x=>({age:x.age,text:x.text,kind:x.kind})),decision:run.currentDecision?{id:run.currentDecision.id,prompt:run.currentDecision.prompt,choices:run.currentDecision.choices.map((x,index)=>({index,text:x.text}))}:null,cards:run.phase==='card'?run.drawOptions.map(x=>({id:x.id,name:x.displayName,effect:x.effectHint})):null,decisionCount:run.decisionCount,ending:run.ending?{id:run.ending.id,profileId:run.ending.profileId,title:run.ending.title,age:run.ending.age,facts:run.ending.facts.length}:null}:null})}
