...
- 当前发布版本：v0.6.6
- 当前本地版本：v0.6.7（已实现，未提交／未发布）
- 当前 `main` 基线版本：v0.6.6
- 下一规划版本：v0.6.8 住房
- 当前剩余规划范围：v0.6.8—v0.6.10
- `schemaVersion`：11
- `contentRevision`：25
...
当前工作边界：v0.6.0—v0.6.6 已发布；v0.6.7 已在本地 `main` 工作区完成实现和验证，但尚未提交、推送或部署。下一步等待用户试玩与 Git 操作授权，不自动开始 v0.6.8。v0.6.10 仍依赖 v0.6.8—v0.6.9 完成。
...
- v0.6.7 已在 `main@23e40dc` 工作区完成本地实现，尚未提交、推送或部署；发布基线仍是 v0.6.6。版本为 v0.6.7、Schema 11、Content Revision 25，v0.6.6 的同 Schema 活动人生可继续读取。
...
7. v0.6.7：晚年生活，已完成本地实现与自动验证；等待用户试玩及后续 Git 授权。
8. v0.6.8：住房，占位待专项规划；本轮未授权开始。
9. v0.6.9：社会交往，占位待专项规划；依赖 v0.6.8，本轮未授权开始。
...
- 同版本刷新应恢复当前人生及所在开局阶段。
- v0.6.6 的 Schema 11 活动局可原位迁移到 v0.6.7；更旧 schema 仍清除未完成人生，保留人生记录、图鉴、设置、统计、跨局新鲜度和种子记录。
...
保留用户已有改动。v0.6.6 已提交、推送并部署，当前线上仍为 v0.6.6；v0.6.7 只存在于当前未提交工作区。后续文案任务必须先完整阅读 `CopyWriting_Guideline.md`，并按当次授权分别判断提交、推送、PR、合并和部署。

下一窗口接手当前 v0.6.7 工作区时，先读：

```text
roadmap/00-总体策略.md
roadmap/v0.6.6-债务执行与生活后果.md
roadmap/v0.6.7-晚年生活.md
roadmap/v0.6.8-住房.md
```

先用 `git diff` 区分本轮实现与更早的用户文档改动，再复跑 `node tests/run-checks.cjs --profile cards,episodes`。不要自动提交、推送、部署，也不要未经新授权进入 v0.6.8；当前下一步是用户试玩和对本地实现的审阅。
