...
- `main` 开发基线：`a8b37c170e6077dbbdaac061c7ffe182ccc450f9`
- 基线提交说明：`Merge v0.5.9 full-track episodes`
- 开发分支：`codex/v0.5.10-family-education`
- 基线版本：`v0.5.9`
...
v0.5.x 事件簇迁移范围已经完成，v0.5.10的本科之后连续路线仍待下一版本实现。当前分支仍需经过 PR 审阅、用户确认后才能合并和部署；不要自动合并。
...
保留用户已有改动。若 v0.5.10 PR 尚未创建或合并，先确认当前分支、验证和审阅状态；未经用户确认不要合并。若已合并，需要重新读取 Pages 的 `index.html`、`game.js` 和 `data.json` 后才能声称线上版本有效，不能沿用本文件的旧结论。下一轮本科、研究生与就业连续路线必须从当时最新 `main` 新建 `codex/` 分支，并读取本版 `development`、申请记录和 `education.nextStage`，不要重新发明一套割裂状态。
