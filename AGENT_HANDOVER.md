...
保留用户已有改动。v0.6.5 已完成本地实现和定向验证，但尚未提交、推送、合并或部署；当前线上仍应按 v0.6.4 核对。后续文案任务必须先完整阅读 `CopyWriting_Guideline.md`，并按当次授权分别判断提交、推送、PR、合并和部署。

下一窗口处理 v0.6.5 发布或准备 v0.6.6 时，先读：
...
实验分支 `investigate_card_game_mechanics` 不是下一版本基线，不整体合并。v0.6.5 发布前仍以当前分支实际 diff 为准；未经明确授权不得提交、推送、合并或部署。
